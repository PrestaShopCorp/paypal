...
<div class="container-fluid paypal-nav">
    <ul class="nav nav-pills navbar-separator">
        <li {if !$ec_paypal_active && !$ec_card_active}class="active"{/if}><a data-toggle="pill"
                                                                              href="#paypal_conf"><span>{l s='Products' mod='paypal'}</span></a>
        </li>
        <li {if $ec_paypal_active || $ec_card_active}class="active"{/if}><a data-toggle="pill"
                                                                            href="#paypal_params"><span>{l s='Settings' mod='paypal'}</span></a>
        </li>
    </ul>
    <div class="tab-content">
        <div id="paypal_conf" class="tab-pane fade {if !$ec_paypal_active && !$ec_card_active}in active{/if}">
            <div class="box half left">
                ...
                <div class="info">
                    ...
                    <p><i>
                            ...
                            <a target="_blank"
                               href="{$localization|escape:'html':'UTF-8'}">{l s='International > Localization' mod='paypal'}</a>
                        </i></p>
                    <p class="paypal-bold">
            ...
            <div class="box half right">
                <ul class="tick">
                    <li>
                        <span class="paypal-bold">{l s='Target more customers' mod='paypal'}</span><br/>{l s='More than 190 million PayPal active users worldwide' mod='paypal'}
                    </li>
                    <li>
                        <span class="paypal-bold">{l s='Truly global' mod='paypal'}</span><br/>{l s='Access a whole world of customers. PayPal is available in more than 200 markets and in 25 currencies' mod='paypal'}
                    </li>
                    <li>
                        <span class="paypal-bold">{l s='Safer' mod='paypal'}</span><br/>{l s='We can protect your business with our Seller Protection and advanced encryption and fraud prevention tools' mod='paypal'}
                    </li>
                    <li><span class="paypal-bold">{l s='Accept local and international payments' mod='paypal'}</span>
                    </li>
                </ul>
            </div>
            ...
            <div class="active-products">
                ...
                <div class="col-sm-6">
                    <div class="panel">
                        ...
                        <p>
                            {l s='Accept' mod='paypal'}
                            <b>{l s='PayPal' mod='paypal'}</b> {l s='payments, you can optimize your conversion rate.' mod='paypal'}
                        </p>
                        <p>
                            {l s='Fast checkout and fast payment. Make online payments simple.' mod='paypal'}
                            <b>{l s='PayPal customers' mod='paypal'}</b> {l s='can buy from you quickly if they use One Touch' mod='paypal'}&trade;
                        </p>
                        <p>
                            <a target="_blank"
                               href="https://www.paypal.com/webapps/mpp/express-checkout">{l s='More Information' mod='paypal'}</a>
                        </p>
                        <div class="bottom">
                            <img src="{$path|escape:'html':'UTF-8'}/views/img/paypal_btm.png" class="product-img">
                            <a class="btn btn-default pull-right"
                               href="#{*$return_url|escape:'html':'UTF-8'}&method=EXPRESS_CHECKOUT*}"
                               onclick="display_popup('EXPRESS_CHECKOUT',0)">{if $ec_paypal_active}{l s='Modify' mod='paypal'}{else}{l s='Activate' mod='paypal'}{/if}</a>
                        </div>
                    </div>
                </div>
                <div class="col-sm-6">
                    <div class="panel">
                        <img class="paypal-products" src="{$path|escape:'html':'UTF-8'}/views/img/paypal.png">
                        <p>
                            {l s='Accept' mod='paypal'} <b>{l s='credit cards' mod='paypal'}</b>,
                            <b>{l s='debit cards' mod='paypal'}</b> {l s='and' mod='paypal'}
                            <b>{l s='PayPal' mod='paypal'}</b> {l s='payments' mod='paypal'}
                        </p>
                        <p>
                            {l s='Your customers can pay with a selection of local and international ' mod='paypal'}
                            <b>{l s='debit and credit cards.' mod='paypal'}</b> {l s='Make online payments simple.' mod='paypal'}
                            <b>{l s='PayPal customers' mod='paypal'}</b> {l s='can buy from you quickly if they use One Touch' mod='paypal'}&trade;
                        </p>
                        <p><a target="_blank"
                              href="https://www.paypal.com/webapps/mpp/standard">{l s='More Information' mod='paypal'}</a>
                        </p>
                        <div class="bottom">
                            <img src="{$path|escape:'html':'UTF-8'}/views/img/paypal_btm.png" class="product-img">
                            ...
                            <a class="btn btn-default pull-right"
                               href="#{*$return_url|escape:'html':'UTF-8'}&method=EXPRESS_CHECKOUT*}"
                               onclick="display_popup('EXPRESS_CHECKOUT',1)">{if $ec_card_active}{l s='Modify' mod='paypal'}{else}{l s='Activate' mod='paypal'}{/if}</a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="panel parametres">
                <div class="panel-body">
                    <div class="col-sm-8 help-left">
                        {if !$paypal_card}
                            ...
                            <p>
                                {l s='Accept' mod='paypal'}
                                <b>{l s='PayPal' mod='paypal'}</b> {l s='payments, you can optimize your conversion rate.' mod='paypal'}
                                : <b>{$active_products|escape:'html':'UTF-8'}</b>
                            </p>
                            <p>
                                {l s='Fast checkout and fast payment. Make online payments simple.' mod='paypal'}
                                <b>{l s='PayPal customers' mod='paypal'}</b> {l s='can buy from you quickly if they use One Touch' mod='paypal'}&trade;
                    ...
                    <div class="col-sm-3 help-right">
                        ...
                        <a target="_blank"
                           href="https://www.paypal.com/webapps/mpp/contact-us">{l s='Contact us' mod='paypal'}</a>
                    </div>
                </div>
            </div>
        </div>
...
<div style="display: none;">
    <div id="content-fancybox-configuration">
        <form action="{$return_url|escape:'javascript':'UTF-8'}" method="post" id="credential-configuration"
              class="bootstrap">
            ...
            <ul>
                <li>{l s='Access' mod='paypal'} <a target="_blank"
                                                   href="https://developer.paypal.com/developer/applications/">{l s='https://developer.paypal.com/developer/applications/' mod='paypal'}</a>
                </li>
            ...
            <p>
                ...
                <input type="text" id="sandbox_client_id" name="sandbox[client_id]"
                       value="{$PAYPAL_SANDBOX_CLIENTID|escape:'htmlall':'UTF-8'}"/>
            </p>
            <p>
                <label for="sandbox_secret">{l s='Secret' mod='paypal'}</label>
                <input type="password" id="sandbox_secret" name="sandbox[secret]"
                       value="{$PAYPAL_SANDBOX_SECRET|escape:'htmlall':'UTF-8'}"/>
            </p>
            ...
            <p>
                ...
                <input type="text" id="live_client_id" name="live[client_id]"
                       value="{$PAYPAL_LIVE_CLIENTID|escape:'htmlall':'UTF-8'}"/>
            </p>
            <p>
                <label for="live_secret">{l s='Secret' mod='paypal'}</label>
                <input type="password" id="live_secret" name="live[secret]"
                       value="{$PAYPAL_LIVE_SECRET|escape:'htmlall':'UTF-8'}"/>
            </p>
            <hr/>
            <p>
                <button class="btn btn-default"
                        onclick="$.fancybox.close();return false;">{l s='Cancel' mod='paypal'}</button>
                <button class="btn btn-info"
                        name="save_credentials">{l s='Confirm API Credentials' mod='paypal'}</button>
            </p>
        </form>
    </div>
...
<div style="display: none;">
    <div id="content-rounding-settings">
        <form action="{$return_url|escape:'javascript':'UTF-8'}" method="post" id="credential-configuration"
            ...
            <p>
                <button class="btn btn-default"
                        onclick="$.fancybox.close();return false;">{l s='I understand' mod='paypal'}</button>
                <button class="btn btn-info"
                        name="save_rounding_settings">{l s='Change rounding settings' mod='paypal'}</button>
            </p>
        </form>
    </div>
